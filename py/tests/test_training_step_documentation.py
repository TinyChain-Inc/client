...
def test_readme_links_the_training_step_to_its_owning_contract() -> None:
    readme_path = Path(__file__).resolve().parents[1] / "README.md"
    text = readme_path.read_text(encoding="utf-8")

    assert "compile_training_step" in text
    assert "CompiledTrainingStep" in text
    assert "tinychain/autodiff/training_step.py" in text
    assert "tests.autodiff_reference_consumer" not in text
